...
export default function SubscriptionPlans({
  ...
  const canUseTrial = !user
    ? true
    : profile && !profile.stripe_customer_id && !profile.trial_ends_at;
    ...
    try {
      ...
      if (paymentLink) {
        ...
        if (user?.email) {
          ...
          finalPaymentLink = `${paymentLink}${separator}prefilled_email=${encodeURIComponent(
            user.email
          )}`;
        }

        console.log(
          'Opening payment link with prefilled email:',
          finalPaymentLink
        );
