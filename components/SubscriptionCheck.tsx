...
export default function SubscriptionCheck({ children }: SubscriptionCheckProps) {
  ...
  // Simple test - if user has no current_period_end, redirect immediately
  useEffect(() => {
    console.log('SubscriptionCheck: SIMPLE TEST - User:', user?.id)
    console.log('SubscriptionCheck: SIMPLE TEST - Profile:', profile)
    
    if (user && profile && profile.current_period_end === null) {
      console.log('SubscriptionCheck: SIMPLE TEST - Redirecting to pricing!')
  ...
  useEffect(() => {
    if (pathname === '/pricing' && user && profile && !loading) {
      ...
      if (hasValidSubscription) {
        console.log('Subscription check: User has valid subscription, redirecting from pricing to dashboard')
  ...
  useEffect(() => {
    ...
    const checkSubscription = () => {
      console.log('Subscription check: FUNCTION CALLED - Profile data:', profile)
      
      if (!profile) {
        console.log('Subscription check: No profile found, redirecting to pricing')
      ...
      console.log('Subscription check: Has current_period_end:', !!profile.current_period_end)

      // Check if user has a valid subscription
      // A user needs either a valid current_period_end OR be on excluded pages
      const hasValidPeriodEnd = currentPeriodEnd && currentPeriodEnd > now
      const isValid = hasValidPeriodEnd

      console.log('Subscription check: Has valid period end:', hasValidPeriodEnd)
      console.log('Subscription check: Is valid subscription:', isValid)
      ...
      if (!isValid) {
        ...
        if (hasRedirected) {
          console.log('Subscription check: Already redirected, allowing access')
          setHasValidSubscription(true)
          setIsChecking(false)
          return
        }

        console.log('Subscription check: Setting redirect flag and redirecting to pricing')
    ...
    console.log('Subscription check: useEffect triggered, calling checkSubscription')
    checkSubscription()
